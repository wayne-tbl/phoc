...
/*
 * Whether background blur can run at all. Off unless PHOC_BLUR is set so that
 * a build carrying the blur code still boots into a plain, known good session.
 */
static gboolean
phoc_renderer_blur_enabled (PhocRenderer *self)
{
  ...
  if (G_UNLIKELY (enabled < 0)) {
    const char *env = g_getenv ("PHOC_BLUR");

    enabled = (env != NULL && g_strcmp0 (env, "0") != 0);
    if (!enabled)
      g_message ("Background blur disabled, set PHOC_BLUR=1 to enable");
  }
...
phoc_renderer_capture_blur (PhocRenderer *self, PhocOutput *output, PhocRenderContext *ctx)
{
   ...
   *
   * Two intervening attempts derived the region from what had been drawn
   * instead -- the union of every surface's extent, and then of only the
   * opaque ones. Both are recorded in the changelog as failures: the first
   * goes to the whole output whenever a full screen translucent surface is
   * present, the second collapses to a sliver when the first thing drawn is
   * the panel's own background. Neither is worth trying again. Nor is looking
   * for a thin bar surface to subtract: phosh's panel is a *full screen* layer
   * surface that paints a bar and reserves a zone, so there is no bar shaped
   * geometry to find.
   *
  ...
  {
    int slop = (int) ceilf (wlr_output->scale);
    int margin = 3 * slop;
