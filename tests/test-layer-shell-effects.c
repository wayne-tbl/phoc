...
{
  PhocTestLayerSurface *ls_green;
  struct zphoc_furios_blur_layer_surface_v1 *blur_surf;
  ...
  blur_surf = zphoc_furios_layer_shell_effects_v1_get_blur_layer_surface (
    globals->furios_layer_shell_effects, ls_green->layer_surface);
  g_assert_nonnull (blur_surf);

  zphoc_furios_blur_layer_surface_v1_set_blur (blur_surf, 60);
  wl_surface_commit (ls_green->wl_surface);
  wl_display_roundtrip (globals->display);
  ...
  zphoc_furios_blur_layer_surface_v1_destroy (blur_surf);
  phoc_test_layer_surface_free (ls_green);
  phoc_assert_screenshot (globals, "empty.png");
